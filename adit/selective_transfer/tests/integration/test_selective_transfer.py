...
def test_unpseudonymized_urgent_selective_transfer(
    ...
    Access.objects.create(access_type="src", group=groupDIR, node=DicomNode.objects.get(name="Orthanc Test Server 1"))
    Access.objects.create(access_type="dst", group=groupDIR, node=DicomNode.objects.get(name="Orthanc Test Server 2"))
    ...
    expect(page.get_by_label("Source")).to_have_count(1)
