...
class TransferExecutor:
    ...
    def _modify_dataset(
        ...
        if (
            hasattr(self.transfer_task.job.destination, "dicomserver")
            and self.transfer_task.job.destination.dicomserver.xnat_server
        ):
            session_id = f"{ds.StudyDate}-{ds.StudyTime}"
            project_id = ""
            subject_id = ""
            if self.transfer_task.job.xnat_destination_project_id:
                project_id = self.transfer_task.job.xnat_destination_project_id
            elif trial_protocol_id:
                project_id = trial_protocol_id
            if self.transfer_task.job.xnat_destination_subject_id:
                subject_id = self.transfer_task.job.xnat_destination_subject_id
            elif pseudonym:
                subject_id = pseudonym
            ds.PatientComments = (
                f"Project:{project_id} Subject:{subject_id} Session:{subject_id}_{session_id}"
            )
