...
**ADIT Task Types** (`adit/core/tasks.py`, `adit/mass_transfer/tasks.py`): `process_dicom_task` (queue `dicom`, runs one DICOM task), `process_mass_transfer_task` (queue `mass_transfer`), `queue_mass_transfer_tasks` (queue `default`, defers the tasks of a mass transfer job in the background), `check_disk_space` (daily, warns admins about full destination folders), and `sweep_stale_tasks_periodic` (see below). The shared package (`adit_radis_shared.common.tasks`) supplies `backup_db`, `retry_stalled_jobs` (every 10 minutes), and `broadcast_mail`.

DICOM tasks run in a separate process with a timeout (`DICOM_TASK_PROCESS_TIMEOUT`, 20 minutes; mass transfer tasks use the longer `MASS_TRANSFER_PROCESS_TIMEOUT`). A `RetriableDicomError` triggers a task-level retry with linear backoff: at most `DICOM_TASK_MAX_ATTEMPTS` (3) attempts, waiting `DICOM_TASK_RETRY_WAIT` (2 minutes) plus `DICOM_TASK_LINEAR_WAIT` (2 minutes) per previous attempt, i.e. 2 → 4 → 6 minutes. Transient network failures are retried much faster at the connector level by [stamina](https://stamina.hynek.me/) before a task-level retry is considered. Complete job history and logs are kept in PostgreSQL.

#### Task recovery

A worker claims a task with a single conditional `PENDING → IN_PROGRESS` UPDATE that also stamps the delivering Procrastinate row onto `DicomTask.queued_job`; because Procrastinate delivers at least once, a row arriving for a task that is no longer pending is skipped. If a worker dies mid-task the task stays `IN_PROGRESS`, and the stale task sweep (`adit/core/utils/recovery.py`) repairs it: every `IN_PROGRESS` task whose queue row is gone, finished, or owned by a worker whose heartbeat is older than `DICOM_TASK_STALLED_WORKER_GRACE_SECONDS` (default 30, never lower) is put back to `PENDING` (or `CANCELED` if its job is canceling) and re-queued when the old row will not run again; afterwards the affected jobs are re-evaluated with `post_process()`. The sweep runs at every worker start (`./manage.py sweep_stale_tasks`, executed before `bg_worker`) and periodically via `sweep_stale_tasks_periodic` (`DICOM_TASK_SWEEP_CRON`, default every minute, on the `default` queue). Queue rows themselves are healed by Procrastinate together with `retry_stalled_jobs`.
