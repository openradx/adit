...
static OFLogger dcmrecvLogger =
    OFLog::getLogger("dcmtk.apps." OFFIS_CONSOLE_APPLICATION);

static char rcsid[] = "$dcmtk: " OFFIS_CONSOLE_APPLICATION
                      " v" OFFIS_DCMTK_VERSION " " OFFIS_DCMTK_RELEASEDATE " $";
...
int main(int argc, char *argv[]) {
  ...
  OFBool opt_showPresentationContexts =
      OFFalse; // default: do not show presentation contexts in verbose mode
  OFBool opt_HostnameLookup =
      OFTrue; // default: perform hostname lookup (for log output)

  DcmStorageSCP::E_DirectoryGenerationMode opt_directoryGeneration =
      DcmStorageSCP::DGM_NoSubdirectory;
  DcmStorageSCP::E_FilenameGenerationMode opt_filenameGeneration =
      DcmStorageSCP::FGM_SOPInstanceUID;
  DcmStorageSCP::E_DatasetStorageMode opt_datasetStorage =
      DcmStorageSCP::DGM_StoreToFile;

  OFConsoleApplication app(OFFIS_CONSOLE_APPLICATION,
                           "Simple DICOM storage SCP (receiver)", rcsid);
  OFCommandLine cmd;
  ...
  cmd.addOption("--use-called-aetitle", "-uca",
  ...
  if (app.parseCommandLine(cmd, argc, argv)) {
    ...
    if (cmd.findOption("--verbose-pc")) {
      app.checkDependence("--verbose-pc", "verbose mode",
                          dcmrecvLogger.isEnabledFor(OFLogger::INFO_LOG_LEVEL));
    ...
    if (cmd.findOption("--max-pdu"))
      app.checkValue(cmd.getValueAndCheckMinMax(
          opt_maxPDULength, ASC_MINIMUMPDUSIZE, ASC_MAXIMUMPDUSIZE));
    ...
    if (cmd.findOption("--bit-preserving")) {
      app.checkConflict("--bit-preserving", "--series-date-subdir",
                        opt_directoryGeneration ==
                            DcmStorageSCP::DGM_SeriesDate);
    ...
    /* evaluate (most of) the TLS command line options (if we are compiling with
     * OpenSSL) */
  ...
  if ((opt_configFile != NULL) && (opt_profileName != NULL)) {
    status = storageSCP.loadAssociationConfiguration(opt_configFile,
                                                     opt_profileName);
    if (status.bad()) {
      OFLOG_FATAL(dcmrecvLogger,
                  "cannot load association configuration: " << status.text());
      return EXITCODE_INVALID_ASSOCIATION_CONFIG;
    }
  } else {
    /* report a warning message that the SCP will not accept any Storage SOP
     * Classes */
    OFLOG_WARN(dcmrecvLogger, "no configuration file specified, SCP will only "
                              "support the Verification SOP Class");
  }

  /* specify the output directory (also checks whether directory exists and is
   * writable) */
  status = storageSCP.setOutputDirectory(opt_outputDirectory);
  if (status.bad()) {
    OFLOG_FATAL(dcmrecvLogger,
                "cannot specify output directory: " << status.text());
    return EXITCODE_INVALID_OUTPUT_DIRECTORY;
  }

  OFLOG_INFO(dcmrecvLogger,
             "starting service class provider and listening ...");
